...
void WhiteLevel::Load(const std::string &fileName)
{
	...
	// Load it as int ...
	vector<int> tmp(_dimensions.X * _dimensions.Y * 4, 0);
...
{ 
	...
	vector<int> tmp(_dimensions.X * _dimensions.Y, 0);
...
void Manager::SetSource(const Source &source)
{
	...
	fill(config.SourceName, config.SourceName + Interface::MaxSourceNameLen, 0);
...
void Manager::AddDestination(Destination &destination)
{
	...
	fill(config.DestinationName, config.DestinationName + Interface::MaxDestinationNameLen, 0);
	assert(destName.size() < Interface::MaxDestinationNameLen); // Must fit (incl. terminator).
	copy(destName.begin(), destName.end(), config.DestinationName);
...
void Manager::SetWhiteLevel(WhiteLevel &table)
{
	...
	vector<int> tmp(table.GetDimensions().X * table.GetDimensions().Y *4, 0);
